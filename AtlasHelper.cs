...
using static AtlasHelper.AtlasHelper;
...
namespace AtlasHelper
{
    public class AtlasHelper : BaseSettingsPlugin<AtlasHelperSettings>
    {
        ...
        private Dictionary<MapCacheKey, MapCacheEntry> globalMapCache = new();

        private void UpdateGlobalMapCache(IEnumerable<NormalInventoryItem> visibleMaps, string source)
        {
            foreach (var item in visibleMaps)
            {
                // Only add maps that can give completion
                ...
                var key = new MapCacheKey(areaName, tier);
                var entry = new MapCacheEntry
                {
                    MapAreaName = areaName,
                    MapTier = tier
                };
                globalMapCache[key] = entry;
            }
        }
        ...
        {
            if (GameController.Game.IngameState.IngameUi.MapDeviceWindow.IsVisible)
                RemoveMapsFromCache();

            DrawGlobalMapCacheOnScreen();
            ...
            var allMapItems = GetAllInventoryAndMapDeviceMaps();
            HighlightDuplicateMaps(allMapItems);
        }
        ...
        private void DrawDiagonalProgression()
        {
            ...
            foreach (var item in allMapItems)
            {
                ...
                var tier = mapComponent.Tier;
                if ((!ListAtlasMapsCompleteable.Any(atlasMap => atlasMap.WorldArea.Name == uniqueName) && rarity == ItemRarity.Unique) ||
                    (!ListAtlasMapsCompleteable.Any(atlasMap => atlasMap.WorldArea.Name == area.Name) && rarity < ItemRarity.Unique))
                {
                    continue;
                }
                ...
                if (naturalTier == 17) continue;
                if (linesIgnoreMaps.Contains(area.ToString())) continue;
            ...
            if (highestCompletedTier >= 3)
            {
                ...
                else if (mapsThatGiveCompletion.Any())
                {
                ...
                else if (mapsThatWontGiveCompletion.Any())
                {
        ...
        private bool npcInvMapCanGiveCompletion(ServerInventory.InventSlotItem npcInventSlotItem)
        {
            ...
            else if (bonusComp.Contains(area) && rarity != ItemRarity.Unique)
            {
                if (Settings.Debug)
                {
                    string enchantedInfo = modsComponent.EnchantedMods.Count > 0 
                        ? $"Enchanted ({GetFormattedEnchantedModsFromMods(modsComponent)}) " 
                        : "Map ";
                    var logColor = modsComponent.EnchantedMods.Any(mod => HasDesiredEnchantedModByName(mod.Name)) ? Color.LightGreen : (Color)Settings.IgnoredMaps;
                    LogMessage(enchantedInfo + area + " | " + rarity + " wont give completion because it is already present on the completed areas", 5, logColor);
                }
        ...
        private string GetFormattedEnchantedModsFromMods(Mods modsComponent)
        {
            if (modsComponent?.EnchantedMods == null || modsComponent.EnchantedMods.Count == 0)
                return "";

            var formattedMods = new List<string>();
            foreach (var mod in modsComponent.EnchantedMods)
            {
                if (HasDesiredEnchantedModByName(mod.Name))
                {
                    formattedMods.Add($"****  {mod.Name}  ****"); // Mark desired mods with stars for visibility
                }
                else
                {
                    formattedMods.Add(mod.Name);
                }
            }
            return string.Join(", ", formattedMods);
        }

        private bool HasDesiredEnchantedModByName(string modName)
        {
            if (!Settings.HighlightEnchantedMods || string.IsNullOrEmpty(modName))
                return false;

            // Check against mod names instead of stats descriptions
            if (Settings.EnchantedModHarvestBeasts && modName.Contains("MapMissionDisplayHarvestBeasts"))
                return true;
            if (Settings.EnchantedModHarvestSeeds && modName.Contains("MapMissionDisplayHugeHarvest"))
                return true;
            if (Settings.EnchantedModFlashBreaches && modName.Contains("MapMissionDisplayFlashBreaches"))
                return true;
            if (Settings.EnchantedModHarbinger && modName.Contains("MapMissionDisplayPlayerIsHarbinger"))
                return true;
            return false;
        }
        ...
        private void DrawPlayerInvMaps()
        {
            ...
            if (ingameState.IngameUi.InventoryPanel.IsVisible)
            {
                ...
                UpdateGlobalMapCache(inventoryZone, "Inventory");
                HiglightAllMaps(inventoryZone);
            }
        }

        private void DrawMapDeviceMaps()
        {
            var mapDeviceSlots = GameController.Game.IngameState.IngameUi.MapDeviceWindow.GetChildAtIndex(0).GetChildAtIndex(1);

            if (mapDeviceSlots.IsVisible)
            {
                var mapDeviceItems = new List<NormalInventoryItem>();

                for (int i = 1; i < mapDeviceSlots.ChildCount; i++)
                {
                    var slot = mapDeviceSlots.GetChildAtIndex(i);
                    var mapItem = slot.AsObject<NormalInventoryItem>();
                    if (mapItem != null)
                        mapDeviceItems.Add(mapItem);
                }
                UpdateGlobalMapCache(mapDeviceItems, "MapDevice");
                HiglightAllMaps(mapDeviceItems);
            }
        }

        private void HighlightDuplicateMaps(IList<NormalInventoryItem> items)
        {
            if (!Settings.HighlightDuplicateMaps)
                return;
            ...
            foreach (var group in mapGroups)
            {
                if (group.Count() > 1)
                {
                    foreach (var item in group)
                    {
                        ...
                        Graphics.DrawBox(rect, Settings.HighlightDuplicateMapsColor, 4);
                    }
                }
            }
        }
        ...
        private List<NormalInventoryItem> GetAllInventoryAndMapDeviceMaps()
        {
            var ingameState = GameController.Game.IngameState;
        ...
        private void DrawNpcInvMaps()
        {
            ...
            var allCachedMaps = globalMapCache.Values.ToList();
            List<string> MapAreasInBag = allCachedMaps.Select(m => m.MapAreaName).Distinct().ToList();
        ...
        */
            ...
            if (Settings.EnableDiagonalProgressionHighlight)
            {
                ...
                foreach (var item in items)
                {
                    ...
                    var match = finalMapsToRun.FirstOrDefault(m =>
                        ...
                        m.Item1.WorldArea.Name == areaName &&
                        m.Item2 == tier);
                    ...
                    if (Settings.Debug)
                        LogMessage("Best maps to run " + match.Item1.ToStringBestMapsToRun(), 5, Color.Pink);

                    if (willGiveCompletion)
                    {
                        Graphics.DrawBox(drawRect, Settings.DiagonalProgressionHighlightWillGiveCompletion);
                    }
                    else
                    {
                        Graphics.DrawBox(drawRect, Settings.DiagonalProgressionHighlightWontGiveCompletion);
                    }
                }
            }
        ...
        private void DrawGlobalMapCacheOnScreen()
        {
            if (!Settings.ShowCacheOnScreen)
                return;
            ...
            if (globalMapCache.Count == 0)
            {
                Graphics.DrawText("[Cache] No maps in cache.", new Vector2(startX, startY), Color.LightGray, 16);
                return;
            }

            float y = startY;
            foreach (var entry in globalMapCache.Values)
            {
                string text = $"[Cache] Area: {entry.MapAreaName}, Tier: {entry.MapTier}";
                Graphics.DrawText(text, new Vector2(startX, y), color, 16);
                y += lineHeight;
            }
        }
        private void RemoveMapsFromCache()
        {
            var ingameState = GameController.Game.IngameState;
            var mapDeviceWindow = ingameState.IngameUi.MapDeviceWindow;
            ...
            try
            {
                var mapDeviceSlotPanel = mapDeviceWindow.GetChildAtIndex(7).GetChildAtIndex(7);
                for (int i = 1; i < mapDeviceSlotPanel.ChildCount; i++)
                {
                    var mapSlotElement = mapDeviceSlotPanel.GetChildAtIndex(i);
                    var mapInventoryItem = mapSlotElement.AsObject<NormalInventoryItem>();
                    if (mapInventoryItem?.Item != null)
                    {
                        var mapComponent = mapInventoryItem.Item.GetComponent<Map>();
                        var mapAreaName = mapComponent?.Area?.Name;
                        var mapTier = mapComponent?.Tier;
                        if (!string.IsNullOrEmpty(mapAreaName) && mapTier.HasValue)
                        {
                            var key = new MapCacheKey(mapAreaName, mapTier.Value);
                            globalMapCache.Remove(key);
                        }
                    }
                }
            }
        ...
        private List<NormalInventoryItem> GetAllInventoryAndMapDeviceMapsAlways()
        {
            var ingameState = GameController.Game.IngameState;
            var allItems = new List<NormalInventoryItem>();
            ...
            if (stashElement != null && stashElement.VisibleStash != null)
            {
                allItems.AddRange(stashElement.VisibleStash.VisibleInventoryItems);
            }

            // Map device (always try to get the slots, regardless of visibility)
            var mapDeviceWindow = ingameState.IngameUi.MapDeviceWindow;
        ...
        private readonly struct MapCacheKey : IEquatable<MapCacheKey>
        {
            public readonly string AreaName;
            public readonly int Tier;

            public MapCacheKey(string areaName, int tier)
            {
                AreaName = areaName;
                Tier = tier;
            }

            public bool Equals(MapCacheKey other) => AreaName == other.AreaName && Tier == other.Tier;
            public override bool Equals(object obj) => obj is MapCacheKey other && Equals(other);
            public override int GetHashCode() => HashCode.Combine(AreaName, Tier);
        }
    }
}
